...
def init_db():
    ...
    c.execute('CREATE TABLE IF NOT EXISTS reports (id INTEGER PRIMARY KEY AUTOINCREMENT, project_id INTEGER NOT NULL REFERENCES projects(id), worker_name TEXT, raw_text TEXT NOT NULL, translated_text TEXT, extracted_task TEXT, extracted_quantity TEXT, extracted_location TEXT, extracted_date TEXT, matched_schedule_id INTEGER REFERENCES schedule_items(id), confidence_score REAL, review_status TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
